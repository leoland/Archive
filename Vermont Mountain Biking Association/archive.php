...
get_header();
	$bg_position = get_field( 'focus_point' );
		if ( have_posts() ) :
			...
			?>
			<header class="page-header block hero height-<?php the_field( 'height', get_option('page_for_posts', true) ); ?>" style="background-color: gray; background-image: url('<?php echo $background; ?>'); background-position: <?php echo $bg_position ?>;">
