...
//the next two lines update the stylesheet version with the time of day, forcing the browser to fetch it fresh.
		...
		if ( devTime() ) :
			...
			wp_enqueue_style( 'ch2-theme-style', get_template_directory_uri() . '/assets/sass/style.css', array(), '1' );

		endif;

		wp_enqueue_script( 'ch2-navigation', get_template_directory_uri() . '/assets/js/navigation-min.js', array(), '20170303', true );

		wp_enqueue_script( 'fancy', get_template_directory_uri() . '/assets/js/ch2-fancy-min.js', array( 'jquery' ), '10', false );

		wp_enqueue_script( 'ch2-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix-min.js', array(), '20170303', true );
	...
	*/
	...
	function custom_menu_order( $menu_ord ) {
		...
		return array(
			...
			'edit.php', // Posts
			'edit.php?post_type=poi', // Points of interest
			'edit.php?post_type=summer_trails', // Summer Trails
			'edit.php?post_type=winter_trails', // Winter Trails
			'edit.php?post_type=trail_conditions', // Conditions
			'edit.php?post_type=page', // Pages
			'separator2', // second separato
...
.login.login, .login .button-primary {
background-color: rgba(2, 48, 62, 1);
}
...
// Callback function to filter the MCE settings
	function ch2_mce_before_init_insert_formats( $init_array ) {
		...
		$style_formats = array(
			...
			array(
				'title'    => 'teal arrow button',
				'selector' => 'a',
				'classes'  => 'btn-default-o',
				'wrapper'  => false,
			),
			array(
				'title'    => 'white arrow button',
				'selector' => 'a',
				'classes'  => 'btn-alt-o',
				'wrapper'  => false,
			),
...
// Update ACF Google Map API key
	function ch2_acf_init() {
		acf_update_setting( 'google_api_key', 'AIzaSyDy07yrdChdKQt3EnJck0QSCz_VJKAKQFw' );
	}

	add_action( 'acf/init', 'ch2_acf_init' );

// todo playing with custom markers
	add_filter( 'facetwp_map_marker_args', function ( $args, $post_id ) {
		$location_type = get_field( 'vmba_location_type', $post_id );
		if ( $location_type == 'hotel' ) {
			$args['icon'] = get_template_directory_uri() . '/assets/icons/abseiling.svg';
		} elseif ( $location_type == 'restaurant' ) {
			$args['icon'] = get_template_directory_uri() . '/assets/icons/restaurant.svg';
		}
	...
	/**
	 * Add Photog Credit information
	 */

	function ch2_attachment_field_credit( $form_fields, $post ) {
		$form_fields['ch2-photo-credit'] = array(
			'label' => 'Photo Credit',
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'ch2_the_photo_credit', true ),
			'helps' => 'For hero images and backgrounds: If provided, a photo credit will be displayed',
		);
	...
	add_filter( 'attachment_fields_to_edit', 'ch2_attachment_field_credit', 10, 2 );

	/**
	 * Save values of Photo Credit and URL in media uploader
	 *
	 */

	function ch2_attachment_field_credit_save( $post, $attachment ) {
		if ( isset( $attachment['ch2-photo-credit'] ) ) {
			update_post_meta( $post['ID'], 'ch2_the_photo_credit', $attachment['ch2-photo-credit'] );
		}
	...
	add_filter( 'attachment_fields_to_save', 'ch2_attachment_field_credit_save', 10, 2 );
