    ...
    function ch2_sandbox_setup()
{
        ...
        function aw_custom_add_image_size_names($sizes)
    {
        ...
        register_nav_menus(array(
            'menu-1' => esc_html__('Main Nav', 'ch2'),
			'menu-2' => esc_html__('Top Nav', 'ch2'),
			'menu-5' => esc_html__('Footer Useful Info', 'ch2'),
            'menu-3' => esc_html__('Footer Highlights', 'ch2'),
            'menu-4' => esc_html__('Footer Key Topics', 'ch2'),
        ));
...
//ide all the code that adds extra buttons
...
function ch2_mce_before_init_insert_formats($init_array)
{
    ...
    $style_formats = array(
        ...
        array(
            'title' => 'Caret after link',
            'selector' => 'a',
            'classes' => 'caret',
            'wrapper' => false,
        ),
        array(
            'title' => 'Highlight Block Content',
            'block' => 'div',
            'classes' => 'highlighted-block',
            'wrapper' => 'true',
        ),
        array(
            'title' => 'In line Emphasis',
            'inline' => 'span',
            'classes' => 'emphasize',
            'wrapper' => false,
		),
		array(
            'title' => 'Bulleted Paragraph',
            'block' => 'p',
            'classes' => 'bullet-p',
            'wrapper' => false,
		)
...
function ch2_excerpt_more($more)
{
    ...
    return '... <p><a class="moretag" href="' . get_permalink($post->ID) . '">Continue Reading</a></p>';
}

add_filter('excerpt_more', 'ch2_excerpt_more');

if (!devTime()) {
//disable acf menu area
...
function prefix_enqueue_scripts(){
	wp_enqueue_script( 'auto-logout', get_template_directory_uri(). '/assets/js/min/auto-logout.min.js', array('jquery'), '', true);
...
//add_action( 'wp_ajax_push_time_stamp', 'push_time_stamp');
//add_action( 'wp_ajax_pull_time_stamp', 'pull_time_stamp');

function pull_time_stamp() {
	if(empty($_POST) || !isset($_POST)) {
        ajaxStatus('error', 'Nothing to update.');
	}
	else {
			$user = wp_get_current_user();
			$serverstamp = get_user_meta( $user->ID, 'time-stamp' , true );
		}
			echo $serverstamp;
			die();
		}
	...
	function push_time_stamp() {
		...
		if(empty($_POST) || !isset($_POST)) {
			...
			renew_wp_cookie();
				$user = wp_get_current_user();
				$stamp = $_POST['push_stamp'];
				update_user_meta( $user->ID, 'time-stamp', 	$stamp);
				if(get_user_meta( $user->ID, 'time-stamp' , true )){
					echo get_user_meta( $user->ID, 'time-stamp' , true );
				}
				die();
			}
		}
		...
		function helio_idle_cookie_expiration( $expiration, $user_id, $remember ) {
			return $remember ? $expiration : 600000;
		}



		//add_filter( 'auth_cookie_expiration', 'helio_idle_cookie_expiration', 99, 3 );
...
//admin related conditional

function leo_hide_admin_bar_if_non_admin( $show ) {
 ...
 add_filter( 'show_admin_bar', 'leo_hide_admin_bar_if_non_admin', 20, 1 );